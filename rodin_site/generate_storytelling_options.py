...
components_dir = 'src/components/QuemSomos'
...
# 1. TIMELINE
with open(os.path.join(components_dir, 'StorytellingTimeline.tsx'), 'w', encoding='utf-8') as f:
    f.write('''"use client";
      ...
      <div className={styles.timeline}>
        ...
        <div className={styles.node}>
          ...
          <div className={styles.content}>
            ...
            <div className={styles.imageBox} style={{ backgroundImage: "url('/aluno_estudando.jpg')" }}></div>
          </div>
        </div>
...
with open(os.path.join(components_dir, 'StorytellingTimeline.module.css'), 'w', encoding='utf-8') as f:
    f.write('''
.container {
  padding: 5rem 5vw;
...
@media(max-width: 768px) {
  .practicesGrid { flex-direction: column; height: auto; }
  .practiceCard { height: 200px; }
  .line { left: 20px; }
  .dot { width: 40px; height: 40px; font-size: 1rem; }
  .node { gap: 1.5rem; }
}
''')

# 2. BENTO
with open(os.path.join(components_dir, 'StorytellingBento.tsx'), 'w', encoding='utf-8') as f:
    f.write('''"use client";
      ...
      <div className={styles.bentoGrid}>
        ...
        <div className={`${styles.card} ${styles.cardLarge}`} style={{backgroundImage: "url('/aluno_estudando.jpg')"}}>
          <div className={styles.overlay}>
            ...
            <h3>Tudo começa com uma pessoa.</h3>
            <p>Antes de qualquer conteúdo, existe um estudante. Com sua própria história, seus interesses, suas dúvidas. Educar para descobrir o próprio potencial.</p>
          </div>
        </div>
        ...
        <div className={`${styles.card} ${styles.cardDark}`}>
          ...
          <p>Educar é olhar para cada estudante em sua individualidade e despertar o prazer pelo conhecimento.</p>
          <div className={styles.iconsRow}>
            <Eye/><Box/><BookOpen/><PenTool/><Rocket/>
          </div>
        </div>
           ...
           <div className={styles.miniGrid}>
             {principles.slice(0,4).map((p, i) => (
                <div key={i} className={styles.miniItem}>
                  <p.icon size={20} color="var(--rodin-orange)"/>
        ...
        <div className={`${styles.card} ${styles.cardWide}`}>
          <div className={styles.practicesGrid}>
            <div className={styles.pText}>
              ...
              <h3>A Prática</h3>
              <p>É preciso colocar em prática todos os dias.</p>
            </div>
            <div className={styles.pImages}>
              {practices.slice(0,3).map((p, i) => (
                <div key={i} className={styles.pImage} style={{backgroundImage: `url('${p.img}')`}}>
                  <span>{p.title}</span>
                </div>
              ))}
            </div>
...
with open(os.path.join(components_dir, 'StorytellingBento.module.css'), 'w', encoding='utf-8') as f:
    f.write('''
.container { padding: 5rem 5vw; color: var(--rodin-white); border-bottom: 2px solid rgba(255,255,255,0.1); }
.sectionTitle { text-align: center; font-size: 2rem; color: var(--rodin-orange); margin-bottom: 4rem; text-transform: uppercase; font-weight: 800;}
.bentoGrid {
  display: grid;
...
@media(max-width: 1024px) {
  ...
  .practicesGrid { flex-direction: column; }
  .pImages { width: 100%; }
}
@media(max-width: 600px) {
  .bentoGrid { grid-template-columns: 1fr; }
  .cardLarge, .cardDark, .cardLight, .cardWide { grid-column: span 1; }
  .pImages { flex-direction: column; }
}
''')

# 3. TABS
with open(os.path.join(components_dir, 'StorytellingTabs.tsx'), 'w', encoding='utf-8') as f:
    f.write('''"use client";
    ...
    <div className={styles.container}>
      ...
      <div className={styles.tabsLayout}>
        <div className={styles.tabList}>
          {tabs.map((t, i) => (
            <button key={i} className={`${styles.tabBtn} ${active === i ? styles.active : ''}`} onClick={() => setActive(i)}>
              {t}
            </button>
          ))}
        </div>
        ...
        <div className={styles.tabContent}>
          {active === 0 && (
            <div className={styles.panel}>
               <h3>Tudo começa com uma pessoa.</h3>
               <p>Antes de qualquer conteúdo, existe um estudante. Com sua própria história, seus interesses, suas dúvidas e suas possibilidades.</p>
               <img src="/aluno_estudando.jpg" alt="Aluno"/>
            </div>
          )}
          {active === 1 && (
            <div className={styles.panel}>
               <h3>História em construção.</h3>
               <p>Educar é olhar para cada estudante em sua individualidade, despertar o prazer pelo conhecimento e criar oportunidades.</p>
               <img src="/aluno_sorrindo.jpg" alt="Aluno"/>
            </div>
          )}
          {active === 2 && (
            <div className={styles.panel}>
               ...
               <div className={styles.grid}>
                 {principles.map((p,i)=><div key={i} className={styles.box}><h4>{p.title}</h4><p>{p.desc}</p></div>)}
               </div>
            </div>
          )}
          {active === 3 && (
            <div className={styles.panel}>
               <h3>Na Prática.</h3>
               <div className={styles.grid}>
                 {practices.map((p,i)=><div key={i} className={styles.boxImg} style={{backgroundImage: `url('${p.img}')`}}><span>{p.title}</span></div>)}
               </div>
            </div>
          )}
        </div>
...
with open(os.path.join(components_dir, 'StorytellingTabs.module.css'), 'w', encoding='utf-8') as f:
    f.write('''
.container { padding: 5rem 5vw; color: var(--rodin-white); }
.sectionTitle { text-align: center; font-size: 2rem; color: var(--rodin-orange); margin-bottom: 4rem; text-transform: uppercase; font-weight: 800;}
.tabsLayout {
  display: flex;
  gap: 4rem;
...
@media(max-width: 768px) {
  .tabsLayout { flex-direction: column; gap: 2rem; padding: 1.5rem; }
  .tabList { width: 100%; flex-direction: row; flex-wrap: wrap; }
  .tabBtn { flex: 1; min-width: 120px; text-align: center; }
